...
class ProductCategoryControllerTest {
	...
	void editCategoryTest() {
		...
		assertEquals(category.getCategoryName(), categoryController.editCategory(category, categoryId).getCategoryName());
	}
	@Test
	void getCategoryTest() {
		...
		Optional<ProductCategory> categoryExist = Optional.ofNullable(new ProductCategory(1, "Birds", "Some Url"));
		when(categoryService.checkCategory(1)).thenReturn(categoryExist);
		assertEquals(categoryExist.get().getCategoryName(), categoryController.getCategoryById(categoryId).get().getCategoryName());
	}
